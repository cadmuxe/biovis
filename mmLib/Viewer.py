    ...
    def glo_install_properties(self):
        ...
        self.glo_add_property(
            {"name":        "fragment_tube",
             ...
             "type":        "boolean",
             "default":     True,
             "action":      "recompile_fragment"
            })
        self.glo_add_property(
            {"name":        "fragment_select",
             "desc":        "Draw fragment",
             "catagory":    "Show/Hide",
             "type":        "boolean",
             "default":     True,
...
class myGLAtomList(GLAtomList):
    def __init__(self, **args):
        GLAtomList.__init__(self, **args)
        self.fragment_id_list=[]

    def update_fragment_id_list(self, fragment_id_list):
        self.fragment_id_list = fragment_id_list
    ...
    def gldl_install_draw_methods(self):
        ...
        self.gldl_draw_method_install(
            {"name":                "fragment_tube",
             "func":                self.glal_draw_fragment_tube,
             "transparent":         False,
             "visible_property":    "fragment_tube",
             "recompile_action":    "recompile_stick"
                        })
        self.gldl_draw_method_install(
            {"name":                "fragment_select",
             "func":                self.glal_draw_fragment_select,
             "no_gl_compile":       True,
             "transparent":         False,
             "visible_property":    "fragment_select",
             "recompile_action":    "recompile_ball"
                        })
        self.gldl_draw_method_install(
            {"name":                "fragment_not_select",
             "func":                self.glal_draw_fragment_not_select,
             "no_gl_compile":       True,
             "transparent":         False,
             "visible_property":    "fragment_not_select",
             "recompile_action":    "recompile_ball"
                        })
    ...
    def glal_draw_fragment_tube(self):
        """
        onely draw the fragments and their connections
        """
        ## driver optimization
        glr_set_material_rgb = self.driver.glr_set_material_rgb
        glr_tube             = self.driver.glr_tube
        ##
        glr_set_material_rgb(*Colors.COLOR_RGBF["black"])

        for chain in self.glal_iter_chains():
            for connection in chain.get_connection().values():
                p1 = chain.get_fragment(connection[0]).position()
                p2 = chain.get_fragment(connection[1]).position()
                glr_tube(p1, p2, 0.4)

    def glal_draw_fragment_select(self):
        """
        onely draw the fragments and their connections
        """
        ## driver optimization
        ...
        glr_set_material_rgb(*Colors.COLOR_RGBF["green"])
        for chain in self.glal_iter_chains():
            for frag in chain.iter_fragments():
                if int(frag.fragment_id) in self.fragment_id_list:
                    glr_sphere(frag.position(),0.8, 10)

    def glal_draw_fragment_not_select(self):
        """
        onely draw the fragments and their connections
        """
        ## driver optimization
        ...
        glr_sphere           = self.driver.glr_sphere
        ##
        glr_set_material_rgb(*Colors.COLOR_RGBF["blue"])
        for chain in self.glal_iter_chains():
            for frag in chain.iter_fragments():
                if int(frag.fragment_id) not in self.fragment_id_list:
                    glr_sphere(frag.position(),0.4, 10)


class GLChain(myGLAtomList):
    """Visualization object for mmLib.Structure.Chain.
    """
    def __init__(self, **args):
        myGLAtomList.__init__(self, **args)
...
class GLStructure(GLDrawList):
    ...
    def update_fragment_id_list(self, fragment_id_list):
        for chain in self.glo_iter_children():
            if isinstance(chain, GLChain):
                chain.update_fragment_id_list(fragment_id_list)
...
class GLViewer(GLObject):
    ...
    def update_fragment_id_list(self,fragment_id_list):
        for item in self.glo_iter_children():
            if isinstance(item, GLStructure):
                item.update_fragment_id_list(fragment_id_list)
