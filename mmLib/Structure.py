...
class Chain(Segment):
    ...
    def get_connection(self):
        """
        get the connection of Fragments
        """
        if self._connection is None:
            self._connection = {}
            for bond in self.iter_bonds():
                frag1 = bond.get_fragment1()
                frag2 = bond.get_fragment2()
                if frag1 != frag2:
                    tb1 = (frag1.fragment_id, frag2.fragment_id)
                    tb2 = (frag2.fragment_id, frag1.fragment_id)
                    if self._connection.has_key(hash(tb1)) or self._connection.has_key(hash(tb2)):
                        continue
                    else:
                        self._connection[hash(tb1)] = tb1
...
class Fragment(object):
    ...
    def position(self):
        '''
        get the centroid of the Fragment
        '''
        if self._position == None:
            atom_number = len(self.atom_list)
            new_pos = numpy.array([0,0,0])
            for atom in self.atom_list:
                new_pos += atom.position
            self._position = new_pos/atom_number
